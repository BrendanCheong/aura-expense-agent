...
| 7 | **FEAT-004 — Webhook Handler** | The entry point that *calls* the agent. Needs the agent to exist first. Wires dedup → cache fast-path → agent invocation → transaction creation. |
...
| **FEAT-005 before FEAT-004** | The webhook *calls* the agent. Building the webhook first requires stubbing the entire agent interface — wasted effort. |
