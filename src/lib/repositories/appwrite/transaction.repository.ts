...
export class AppwriteTransactionRepository implements ITransactionRepository {
  ...
  async findById(id: string): Promise<Transaction | null> {
    try {
      const row = await this.tablesDb.getRow<TransactionRow>({
        databaseId: DB_ID,
        tableId: TABLE_ID,
        rowId: id,
      });
  ...
  async findByResendEmailId(resendEmailId: string): Promise<Transaction | null> {
    const result = await this.tablesDb.listRows<TransactionRow>({
      databaseId: DB_ID,
      tableId: TABLE_ID,
      queries: [Query.equal('resend_email_id', resendEmailId), Query.limit(1)],
    });
  ...
  async create(data: TransactionCreate): Promise<Transaction> {
    ...
    const row = await this.tablesDb.createRow<TransactionRow>({
      databaseId: DB_ID,
      tableId: TABLE_ID,
      rowId: ID.unique(),
      data: rowData,
  ...
  async update(id: string, data: TransactionUpdate): Promise<Transaction> {
    ...
    const row = await this.tablesDb.updateRow<TransactionRow>({
      databaseId: DB_ID,
      tableId: TABLE_ID,
      rowId: id,
      data: rowData,
